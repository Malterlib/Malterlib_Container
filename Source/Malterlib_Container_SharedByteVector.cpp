...
namespace NMib::NContainer
{
	namespace
	{
		...
		// Keeping the vector header out of the allocation avoids rounding a power-of-two payload up to the next size class.
	...
	// o_pWritable may only be used to fill the allocation before the returned view is shared.
	...
	// Views [_Offset, _Offset + _nBytes); the range must be within the buffer.
		...
		, mp_pOwner(_Other.mp_pOwner)
	{
		...
		// Validate the offset before forming a pointer beyond the view.
		mp_pData = _Other.mp_pData ? _Other.mp_pData + _Offset : nullptr;
	}

	// Clear the source pointer and length as well as its owner so a moved-from view cannot report inaccessible bytes.
	...
	{
		// Retain the source before releasing the destination: they may be the same last owner.
		auto pOwner = _Other.mp_pOwner;

		mp_pData = _Other.mp_pData;
		mp_nBytes = _Other.mp_nBytes;
		mp_pOwner = fg_Move(pOwner);
	...
	// An empty view can still retain a buffer.
	...
	// Appends adjacent bytes with the same owner; an empty _Next succeeds unchanged. Otherwise returns false without changing either view.
