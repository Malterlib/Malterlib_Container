...
namespace NMib::NContainer
{
	...
	{
		mint m_SegmentSize = 64;                                       // Fixed segment size (slots per segment) when m_SegmentBytes == 0
		mint m_SegmentBytes = 0;                                       // Target segment size in bytes (0 = use m_SegmentSize)
		bool m_bAdaptive = true;                                       // Enable adaptive gap placement
		bool m_bShrink = true;                                         // Allow shrinking on underflow
		mint m_MinSegments = 1;                                        // Minimum number of segments
	...
	constexpr CPackedMapOptions fg_PackedMapOptionsScanOriented(mint _SegmentSize = 64, mint _SegmentBytes = 0)
	{
