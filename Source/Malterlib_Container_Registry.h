...
#		endif
	...
	public:
		...
		void f_Feed(tf_CStream &_Stream, TCRegistryStringTable<tf_CStr, tf_CIndex> &_StringTable) const
		{
			_StringTable.f_FeedString(_Stream, m_Name);
		}

		template <typename tf_CStream, typename tf_CStr, typename tf_CIndex>
		void f_Consume(tf_CStream &_Stream, TCRegistryStringTable<tf_CStr, tf_CIndex> const &_StringTable)
		{
		...
		bint f_GetParsed(ERegistryWhiteSpaceLocation _Location) const
		{
			return false;
		}
		void f_SetParsed(ERegistryWhiteSpaceLocation _Location, bint _bParsed)
		{
		}
		...
		void f_SetWhiteSpace(ERegistryWhiteSpaceLocation _Location, t_CKeyStr const &_Str)
		{
		}

		t_CKeyStr f_GetWhiteSpace(ERegistryWhiteSpaceLocation _Location) const
		{
			return t_CKeyStr();
		}
	};
...
/*				else if (m_Sequence < 0)
	...
	private:
		...
		t_CWhiteSpaceStr m_WhiteSpace[ERegistryWhiteSpaceLocation_Max];
...
/*				else if (m_Sequence < 0)
		...
		void f_SetWhiteSpace(ERegistryWhiteSpaceLocation _Location, t_CWhiteSpaceStr const &_Str)
		{
			m_WhiteSpace[_Location] = _Str;
		}

		t_CWhiteSpaceStr const &f_GetWhiteSpace(ERegistryWhiteSpaceLocation _Location) const
		{
	...
	class TCRegistry
	{
		...
		NIntrusive::TCAVLLink<> m_ChildLink;
		typedef NIntrusive::TCAVLTree<&TCRegistry::m_ChildLink, CAVLCompare_TCRegistry> CTree;
		...
		struct CChildren_PreserveOrder
		{
			...
			typedef NMib::NIntrusive::TCDLinkList
				<
					...
					, NMib::NIntrusive::TCDLinkAggregateListNoPrevPtrList<NMib::NMemory::CDefaultAllocator>
					, false
	...
	private:
		...
		public:
			...
			t_CKeyStr m_WhiteSpace[ERegistryWhiteSpaceLocation_Max];
			...
			t_CKeyStr const &f_GetWhiteSpace(ERegistryWhiteSpaceLocation _Location) const
			{
				return m_WhiteSpace[_Location];
			}
...
/*
		...
		void fp_SetParsedWhiteSpace(ERegistryWhiteSpaceLocation _Location, t_CKeyStr2 const &_Str)
		{
...
/*				t_CTempStr Space;
			...
			if (ESupportWhiteSpace)
			{
				auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_BeforeKey);

				if ((!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_BeforeKey)) && !fsp_OnlyWhiteSpace(WhiteSpace))
				{
			...
			if (!DataStr.f_IsEmpty() || !bHasScope || bForceEscaped )
			{
				if (ESupportWhiteSpace)
				{
					auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_Between);

					if (!WhiteSpace.f_IsEmpty() || (_pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_Between) && (!_pReg->m_Children.m_Tree.f_IsEmpty() || !DataStr.f_IsEmpty())))
					{
			...
			if (ESupportWhiteSpace)
			{
				auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_After);

				if (!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_After))
					_Stream += WhiteSpace;
				else
					_Stream += NewLine;
			}
			...
			if (bHasScope)
			{
				if (ESupportWhiteSpace)
				{
					auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_BeforeChildScopeStart);

					if ((!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_BeforeChildScopeStart)) && !fsp_OnlyWhiteSpace(WhiteSpace))
						_Stream += WhiteSpace;
					else
						_Stream += PreData;
				}
				...
				if (ESupportWhiteSpace)
				{
					auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_AfterChildScopeStart);

					if (!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_AfterChildScopeStart))
						_Stream += WhiteSpace;
					else
						_Stream += NewLine;
				}
				...
				if (ESupportWhiteSpace)
				{
					auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_BeforeChildScopeEnd);

					if ((!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_BeforeChildScopeEnd)) && !fsp_OnlyWhiteSpace(WhiteSpace))
						_Stream += WhiteSpace;
					else
						_Stream += PreData;
				}
				...
				if (ESupportWhiteSpace)
				{
					auto WhiteSpace = _pReg->m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_AfterChildScopeEnd);

					if (!WhiteSpace.f_IsEmpty() || _pReg->m_Key.f_GetParsed(ERegistryWhiteSpaceLocation_AfterChildScopeEnd))
						_Stream += WhiteSpace;
					else
						_Stream += NewLine;
				}
	...
	public:
		...
		void fp_Copy(TCRegistry<tf_CKeyStr, tf_CData, tf_CRegistryKey, tf_bPreserveOrder> const &_Source)
		{
			...
			if (m_Key.ESupportWhiteSpace)
			{
				...
				for (mint i = 0; i < ERegistryWhiteSpaceLocation_Max; ++i)
				{
					...
					m_Key.f_SetParsed((ERegistryWhiteSpaceLocation)i, _Source.m_Key.f_GetParsed((ERegistryWhiteSpaceLocation)i));
				}
			}
		...
		template <typename t_CKeyStr2>
		bint f_IsValidWhiteSpace(ERegistryWhiteSpaceLocation _Location, t_CKeyStr2 const &_Str)
		{
		...
		template <typename t_CKeyStr2>
		void f_SetWhiteSpace(ERegistryWhiteSpaceLocation _Location, t_CKeyStr2 const &_Str)
		{
			DMibFastCheck(f_IsValidWhiteSpace(_Location, _Str));
			m_Key.f_SetWhiteSpace(_Location, _Str);
		}
		...
		bool f_HasScope() const
		{
			return !m_Children.m_Tree.f_IsEmpty() ||
				(
					...
					&&
					(
						!fsp_OnlyWhiteSpace(m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_AfterChildScopeStart))
						|| !fsp_OnlyWhiteSpace(m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_BeforeChildScopeStart))
						|| !fsp_OnlyWhiteSpace(m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_BeforeChildScopeEnd))
						|| !fsp_OnlyWhiteSpace(m_Key.f_GetWhiteSpace(ERegistryWhiteSpaceLocation_AfterChildScopeEnd))
					)
				)
			;
		}

		auto f_GetWhiteSpace(ERegistryWhiteSpaceLocation _Location) const -> decltype(fp_GetKeyHelper().f_GetWhiteSpace(_Location))
		{
		...
		TCRegistry *f_InsertChild(t_CKeyStr _Name, NMib::NStorage::TCUniquePointer<TCRegistry> &&_Child)
		{
		...
		void f_FeedWithStringTable(t_CStream &_Stream) const
		{
			...
			TCRegistryStringTable<t_CKeyStr, uint32> StringTable;
			// Build string table;
...
namespace NMib::NStream
{
	...
	class TCBinaryStreamTypeReferenceNamed<t_CNamedStream, NMib::NStream::TCNamedStreamInfo<t_CKeyStrInfo, NContainer::TCRegistry<t_CKeyStr, t_CData, t_CRegistryKey, t_bPreserveOrder>> >
	{
	public:
		typedef NContainer::TCRegistry<t_CKeyStr, t_CData, t_CRegistryKey, t_bPreserveOrder> CDataRegistry;
	...
	class TCBinaryStreamTypeReference<NContainer::TCRegistry<t_CKeyStrStream, t_CDataStream, t_CRegistryKeyStream, t_bPreserveOrder>, NMib::NStream::TCNamedStreamInfo<t_CKeyStr, t_CData> >
	{
		typedef NContainer::TCRegistry<t_CKeyStrStream, t_CDataStream, t_CRegistryKeyStream, t_bPreserveOrder> CStreamRegistry;
	public:
	...
	template <typename t_CKeyStrStream, typename t_CDataStream, typename t_CRegistryKeyStream, typename t_CKeyStr, typename t_CData, bool t_bPreserveOrder>
	class TCBinaryStreamTypeReference<NContainer::TCRegistry<t_CKeyStrStream, t_CDataStream, t_CRegistryKeyStream, t_bPreserveOrder>, NMib::NStream::TCNamedStreamInfo<t_CKeyStr, t_CData const> >
	{
		typedef NContainer::TCRegistry<t_CKeyStrStream, t_CDataStream, t_CRegistryKeyStream, t_bPreserveOrder> CStreamRegistry;
	public:
