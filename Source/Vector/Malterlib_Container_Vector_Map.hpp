...
namespace NMib::NContainer
{
	...
	{
		mint Len = _This.f_GetLen();
		auto pSrc = _This.f_GetArray();

		using CResult = decltype(NPrivate::fg_InvokeVectorIteratorFunctor(_fTransform, fg_ForwardAs<tf_CThis>(pSrc[0]), mint(0), fg_Forward<tf_CThis>(_This)));
