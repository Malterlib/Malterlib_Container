...
namespace NMib::NStream
{
	namespace NPrivate
	{
		// Detects streams that can adopt a byte buffer by reference instead of copying it
		// (segmented storage streams). The byte vector glue below uses this to turn rvalue
		// feeds into segment adoption while all other streams keep copying
